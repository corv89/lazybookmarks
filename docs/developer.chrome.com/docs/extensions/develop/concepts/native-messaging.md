...
Extensions can exchange messages with native applications using an API that is similar to the other [message passing APIs](/docs/extensions/develop/concepts/messaging). Native applications that support this feature must register a *native messaging host* that can communicate with the extension. Chrome starts the host in a separate process and communicates with it using standard input and standard output streams.
...
Name of the native messaging host. Clients pass this string to [`runtime.connectNative()`](/docs/extensions/reference/api/runtime#method-connectNative) or [`runtime.sendNativeMessage()`](/docs/extensions/reference/api/runtime#method-sendNativeMessage). This name can only contain lowercase alphanumeric characters, underscores and dots. The name can't start or end with a dot, and a dot can't be followed by another dot.
...
**Note:** In versions of Chrome earlier than Chrome 146, Google Chrome for Testing used the same locations as Google Chrome.
...
The first argument to the native messaging host is the origin of the caller, usually `chrome-extension://[ID of allowed extension]`. This allows native messaging hosts to identify the source of the message when multiple extensions are specified in the `allowed_origins` key in the [native messaging host manifest](#native-messaging-host).
...
When a messaging port is created using [`runtime.connectNative()`](/docs/extensions/reference/api/runtime#method-connectNative) Chrome starts native messaging host process and keeps it running until the port is destroyed. On the other hand, when a message is sent using [`runtime.sendNativeMessage()`](/docs/extensions/reference/api/runtime#method-sendNativeMessage), without creating a messaging port, Chrome starts a new native messaging host process for each message. In that case the first message generated by the host process is handled as a response to the original request, and Chrome will pass it to the response callback specified when [`runtime.sendNativeMessage()`](/docs/extensions/reference/api/runtime#method-sendNativeMessage) is called. All other messages generated by the native messaging host in that case are ignored.

## Connecting to a native application

Sending and receiving messages to and from a native application is very similar to cross-extension messaging. The main difference is that [`runtime.connectNative()`](/docs/extensions/reference/api/runtime#method-connectNative) is used instead of [`runtime.connect()`](/docs/extensions/reference/api/runtime#method-connect), and [`runtime.sendNativeMessage()`](/docs/extensions/reference/api/runtime#method-sendNativeMessage) is used instead of [`runtime.sendMessage()`](/docs/extensions/reference/api/runtime#method-sendMessage).

To use these methods, the "nativeMessaging" permission must be [declared](/docs/extensions/develop/concepts/declare-permissions) in your extensions's manifest file.
...
The following example creates a [`runtime.Port`](/docs/extensions/reference/api/runtime#type-Port) object that's connected to native messaging host `com.my_company.my_application`, starts listening for messages from that port and sends one outgoing message:
...
Use [`runtime.sendNativeMessage`](/docs/extensions/reference/api/runtime#method-sendNativeMessage) to send a message to the native application without creating a port, e.g.:
...
-   Is the manifest in the right directory and with the correct name? See [native messaging host location](#native-messaging-host-location) for the expected formats.
...
The native messaging host was not found in the Windows registry. Double-check using `regedit` whether the key was really created and matches the required format as documented at [native messaging host location](#native-messaging-host-location).
...
-   Make sure that all output in `stdout` adheres to the [native messaging protocol](#native-messaging-host-protocol). If you want to print some data for debugging purposes, write to `stderr`.
